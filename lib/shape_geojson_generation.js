 ...
 * options.password: the db user's password to use.  Required if you don't use .pgpass file
...
exports.shape_service = function shape_service(options,cb){
    ...
    function seg_geom_service(req,res,next,_cb){
        ...
        _.each(dynamic_where_clause
               ,function(v,k){
                   if(req.query[k] !== undefined){
                       _w_c.push([v.lhs,req.query[k]].join(v.comp))
                   }
               });
        ...
        var _with_clause //= req.params.with_clause)
        ...
        var pn = geom_utils.precision(req.params.zoom) || 1;
        ...
        var data = {"type":"FeatureCollection",
                    "features":[]};
        //console.log('row_handler: '+(req.params.row_handler === undefined))
        var rh = (req.params.row_handler === undefined) ?
            row_handler(select_properties,id_col,data,req,res,next)
            : req.params.row_handler
