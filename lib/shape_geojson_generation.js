...
exports.shape_service = function shape_service(options,cb){
    ...
    function seg_geom_service(req,res,next){
        ...
        var bbox = geom_utils.get_bbox_with_format(req,'bounding_area')
        var _where_clause =  req.param('where_clause')
        if(_where_clause !== undefined && where_clause !== undefined) {
            _where_clause = where_clause + ' AND ' + _where_clause
        }else if(_where_clause === undefined ){
            _where_clause = where_clause
        }

        // add the ability to specify a with clause
        var _with_clause = req.param('with_clause')
        if(_with_clause !== undefined && with_clause !== undefined) {
            _with_clause = with_clause + ',' + _with_clause
        }else if(_with_clause === undefined ){
        ...
        if(bbox !== undefined){
            geoquery += ',bounding_area' // add the with clause alias

            geoquery += ' WHERE st_intersects('+geom+',bounding_area.geom)'
            if(_where_clause){
                geoquery += ' AND ' + _where_clause
            }
        }else{
            if(_where_clause){
        ...
        var data = {"type":"FeatureCollection",
                    "features":[]};
        var rh = row_handler(select_properties,id_col,data,req,res,next)
        var eh = cb === undefined
               ? writing_end_handler(data,req,res,next)
               : cb_end_handler(cb,data,req,res,next)
