...
exports.shape_service = function shape_service(options){
    ...
    if(_.intersection(_.keys(options),required_options).length !== required_options.length ){
        ...
        console.log( _.intersection(_.keys(options),required_options) )
    ...
    var end_handler=function(data,req,res,next){
        return function(){
            ...
            console.log('dumping data to json: '+data.features.length)
    ...
    function seg_geom_service(req,res,next){
        ...
        var bb = geom_utils.bbox_from_xyz({row: req.param('row')
                                          ,column: req.param('column')
                                          ,zoom: req.param('zoom')
                                          });
        if( bb === undefined)
            return next(new Error('bounding box required'))
        var capture
        if((capture = geo_regex.exec(bb))){
            var p1 = capture[1]
            var q1 = capture[2]
            var p2 = capture[3]
            var q2 = capture[4]
            var pn = geom_utils.precision(req.param('zoom'));
            var overlap = 1/Math.pow(10,pn);
            var tolerance = overlap; // this is too simple for my taste: 1/Math.pow(10,pn-1);
               var bbox = "ST_Expand(ST_Envelope(ST_GeomFromEWKT('SRID=4326;POLYGON(("
                   +p1+' '+q1+','
                   +p1+' '+q2+','
                   +p2+' '+q2+','
                   +p2+' '+q1+','
            ...
            var geoquery=
                'SELECT '+propcols+', st_asgeojson(ST_Simplify((ST_Dump(ST_Intersection('+bbox+','+geom+'))).geom,'+tolerance+'),'+pn+') as geojson'
                         + ' FROM '+table+' '
            if(options.alias) geoquery += options.alias + ' '
